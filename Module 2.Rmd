...
library("ggplot2")
ggplot(x, aes(x=TripleClub, y = `How many hours a week do you spend exercising?`)) + geom_boxplot() + geom_jitter(width=0.15, size=3, colour = "blue") + theme_bw(base_size=32) + labs(x="join clubs (3 or more)", y="hours of sports played")
```
...
library("ggplot2")
ggplot(x, aes(x=`Do you currently live with your parents?`, y = `On average, how many hours per week did you spend on university work last semester?`)) + geom_boxplot() + geom_jitter(width=0.15, size=3, colour = "blue") + theme_bw(base_size=32) + labs(x="live  with parents", y="hours of study")
```
...
**4. Observed Test Statistic:** $$t_0 = -0.0.113$$
```{r}
deg_free = nY+nN-2
p_val = 2 * (1 - pt(abs(t0), deg_free))
p_val
...
**5. P Value:** $$P(X^2_1 \geq -0.645) = 0.910$$
